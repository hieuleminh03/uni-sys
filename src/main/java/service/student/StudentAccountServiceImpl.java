...
@Slf4j
public class StudentAccountServiceImpl {
    ...
    /**
     * Get the current student's account information
    ...
    public BaseResponse<AccountInformationResponse> getAccountInformation() {
        try {
            ...
            log.error("Failed to retrieve student account information", e);
    ...
    public BaseResponse<String> changePassword(ChangePasswordRequest request) {
        try {
            ...
            log.info("Password changed for student: {}", auth.getName());
